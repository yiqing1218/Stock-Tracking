...
窗口顶部固定提供“大盘监看、自选股票、股票详情、量化策略、消息提示”五个入口。量化策略和消息提示目前为预留页，不执行策略或发送提醒。“股票详情”只承接从自选列表打开的证券；尚未从自选进入时会保留完整分析框架，但不加载任何证券内容。

## 大盘监看

- 监看上证指数、深证成指、创业板指、沪深300、上证50、中证500、科创50和北证50，单个指数失败不会拖空整页。
- 汇总全A上涨、下跌、平盘、涨跌停近似家数、中位涨幅和成交额。
- 展示领涨行业、涨幅前列和跌幅前列股票；行情按北京时间更新并保留短缓存。
