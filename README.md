...
窗口顶部固定提供“大盘监看、条件荐股、自选股票、股票详情、量化策略与回测、消息提示”六个入口。“股票详情”只承接从自选列表打开的证券；尚未从自选进入时会保留完整分析框架，但不加载任何证券内容。
...
- 全市场扫描只计算被选中的1至5项指标，单股数据用完即释放，不生成常驻的全市场480列宽表。
...
原“资金与筹码”和“公司与财务”已合并，在同一页面通过二级导航切换：主力资金、筹码分布、主要股东、企业概况、主营业务、财务信息、F10深度。
...
- 行情规则仅在北京时间A股交易时段每30秒检查一次，只保留小型规则记录、最近20个价格点和触发状态。
