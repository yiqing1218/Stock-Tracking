...
- 展示行业与概念板块完整涨跌列表，不展示板块内个股；同时展示正确的涨幅榜与跌幅榜。
...
原合并页已经拆分。详情页结构为“行情、指标、简况（F10）、资金、筹码、盘口、财务、企业公告/新闻/资讯、行情提醒设置”。
...
- “量化”子页预留；“回测”子页支持安全公式 `ENTRY` / `EXIT`、固定持有期、止盈止损、仓位、单股/自选组/全市场范围、最大持仓、手续费、印花税、滑点和复权方式。
- 撮合按信号确认后的下一交易日开盘执行，处理A股T+1、整手、停牌、涨跌停、ST特殊涨跌幅和上市前5个交易日限制。
- 输出累计/年化收益、最大回撤、Sharpe、Sortino、Calmar、胜率、盈亏比、平均持仓时间、连续亏损、年度/月度收益、沪深300超额收益、交易记录和信号原始数据。
...
src/astock_monitor/
  ...
  historical_store.py SQLite历史仓库与三阶段表迁移
